// Netlify Function — send-erp-email
// Reçoit { email, erpDocument } depuis le client après paiement réussi.
// 1. Sauvegarde le document ERP dans Netlify Blobs (pour re-téléchargement)
...
//   URL                  → injecté automatiquement par Netlify en production
...
exports.handler = async (event) => {
  ...
  let email, erpDocument;
  try {
    ({ email, erpDocument } = JSON.parse(event.body || '{}'));
  } catch {
    return { statusCode: 400, body: JSON.stringify({ error: 'Corps de requête invalide' }) };
  }

  if (!email || !erpDocument?.metadata?.reference) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Email et document ERP requis' }) };
  }
  ...
  // ─── 1. Sauvegarde dans Netlify Blobs ───────────────────────────────────────
  // Permet de retrouver le document même si le localStorage est vidé
  try {
    const store = getStore({ name: 'erp-documents', consistency: 'strong' });
    await store.set(reference, JSON.stringify(erpDocument), {
      metadata: { email, savedAt: new Date().toISOString() },
    });
  } catch (err) {
    // Non bloquant — on continue avec l'email même si la sauvegarde échoue
    console.error('Netlify Blobs save error:', err.message);
  }
  ...
  if (!resendKey) {
    // Clé non encore configurée — architecture prête, envoi suspendu
    console.warn('RESEND_API_KEY non configurée — document sauvegardé, email non envoyé');
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ success: true, warning: 'email_not_sent_no_api_key' }),
    };
  }
  ...
  <div style="max-width:600px;margin:0 auto;padding:24px 16px;">
    ...
    <div style="background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 10px 10px;padding:28px 24px;">
      ...
      <div style="text-align:center;margin:28px 0;">
        <a href="${redownloadUrl}"
           style="display:inline-block;background:#b20f11;color:#fff;padding:14px 32px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px;letter-spacing:0.02em;">
          Télécharger mon ERP (PDF)
        </a>
        <p style="margin:10px 0 0;font-size:12px;color:#9ca3af;">
          Lien valable à tout moment — conservez cet email précieusement.
        </p>
      </div>
      ...
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:14px;font-size:13px;color:#991b1b;margin-bottom:20px;">
        <strong>Rappel :</strong> Ce document doit être annexé au compromis de vente ou au contrat de bail
        (art. L125-5 Code de l'Environnement). Il est valable <strong>6 mois</strong> à compter de sa date d'établissement.
      </div>
